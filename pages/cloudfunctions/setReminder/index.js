Page({
  ...
  submitReminder: function () {
    ...
    // 输入验证
    if (reminderType === 'once' && (!onceDate || !onceTime)) {
        wx.showToast({
            title: '请选择一次性提醒的日期和时间',
            icon: 'none'
        });
        return;
    }
    if (reminderType === 'periodic' && (!periodicTime)) {
        wx.showToast({
            title: '请选择周期性提醒的时间',
            icon: 'none'
        });
    ...
    wx.cloud.callFunction({
        ...
        success: res => {
            if (res.result.dbResult && res.result.dbResult._id) {
                wx.showToast({
                    title: '提醒设置成功，已保存到数据库',
                    icon: 'success'
                });
            } else {
                wx.showToast({
                    title: '提醒设置成功，但保存到数据库失败',
                    icon: 'none'
                });
            }
        },
